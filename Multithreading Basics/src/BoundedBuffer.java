...
/**
 * Illustrates how to use a generic type, basic synchronization, and the
 * use of {@link Thread#wait()} and {@link Thread#notifyAll()} to create
 * a thread-safe data structure. Uses the produce-consumer model to demo
 * this data structure with multiple threads.
 *
...
public class BoundedBuffer<E> {
	 ...
	 *
	 *    0 1
	 *   7   2   versus   0 1 2 3 4 5 6 7 8
	 *   6   3
	 *    5 4
	 *
	 * We add elements at the end of the buffer, increment the "end" index.
	 * We remove elements from the "beginning" of the buffer, and increment
	 * the "beg" index. These indices wrap around the circle as necessary.
	 * If the "beg" and "end" indices are the same, then we have run out of
	 * room in the buffer.
	 */
	 ...
	 *
	 * We use an {@link Object} array since we cannot create arrays of
	 * generic types.
	 */
	 ...
	 * efficient implementation, but demonstrates that synchronized methods
	 * can call other synchronized methods since the thread already holds
	...
	/**
	 * Removes and returns an element from the buffer. If the buffer is
	 * empty, waits until there is an element to retrieve.
	 *
